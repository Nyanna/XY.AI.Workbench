...
class EditMarksTool(ToolDefinition):
    ...
    input_schema = {'type': 'object', 'strict': True, 'additionalProperties': False, 'properties': {'path': {'type': 'string', 'description': 'Absolute path to the target file.'}, 'begin_marker': {'type': 'string', 'minLength': 10, 'maxLength': 30, 'description': "Unique substring (10-30 chars) marking the beginning of the block. Must occur exactly once in the file. Must end before 'end_marker' begins (no overlap). IMPORTANT: 'end_marker' must not appear anywhere within this string. Choose a distinctive short phrase, e.g. a full line of code or text."}, 'content': {'type': 'string', 'description': "Replacement text that will replace everything from the start of 'begin_marker' to the end of 'end_marker', inclusive."}, 'end_marker': {'type': 'string', 'minLength': 10, 'maxLength': 30, 'description': "Unique substring (10-30 chars) marking the end of the block. Must occur exactly once in the file, at a position strictly after 'begin_marker' ends. Must NOT be a substring of 'begin_marker'. Choose a distinctive short phrase, e.g. a full line of code or text."}, 'exact': {'type': 'boolean', 'description': "If true, 'begin_marker'/'end_marker' must match whitespace exactly. If false (default), whitespace runs match any amount/kind of whitespace.", 'default': False}}, 'required': ['path', 'begin_marker', 'end_marker', 'content']}
    ...
    def handle(self, ctx: ToolContext) -> ToolResult:
        ...
        try:
            result = edit_marks(path=args['path'], begin_marker=args['begin_marker'], end_marker=args['end_marker'], content=args['content'], exact=args.get('exact', False))
