In `/home/user/xyan/xy.ai.workbench/mcpc/src/xy/ai/mcpc/tools/ast/outline.py`:
- OutlineResult: soll eindeutig differenzieren: in "files" kommen nur erfolgreiche Ergebnisse ein "failed" enthält dann eine Liste von Error Objekten. Dadurch wird eine Verwendung von "all_ok" und in FileOutline "ok", "error" obsolete.
- FileOutline: Ändert seine Struktur. Es werden alle direkten Kinder des Wurzelknotens ausgegeben mit Typ, Qualified Name(soweit anwendbar und eindeutig), Zeilennummer oder Zeilenbereich, Signatur mit Docstring. Rekursiv werden verschachtelte Klassen ebenfalls aufgeschlüsselt. Ziele ist eine kompakte aber zur Orientierung ausreichen tokeneffiziente Repräsentation. Dieser Baum soll typisiert sein, kein dict.
Der MCP Output kann daraus dann wieder asdict daraus abgeleitet und formatiert werden.
SystemInit:  id=f4c75035-7db8-4495-9da7-2c48e7e971b2, cwd=/home/user/xyan/xy.ai.workbench, model=claude-sonnet-5
Control Request:
```yaml
id: 43d0-9
toolName: read_file
arguments:
  ...
  reason: "Aktuellen Stand der Datei lesen, um die Änderungen an OutlineResult und FileOutline umzusetzen."
```
/answer 43d0-9 allow
ReasoningToken: 10
Control Request:
...
arguments:
  ...
  reason: "Verstehen der core-Hilfsfunktionen (Located, qualified_name, signature, docstring, locate_all, load, Importtypen) zur Neugestaltung von FileOutline."
```
/answer 43d0-a allow
ReasoningToken: 49
Control Request:
...
arguments:
  ...
  end: outline.py – compact structural overview of Python files.\"\"\"" placeholder
  content: PLACEHOLDER
  reason: placeholder to avoid mistakes; will redo with correct markers
```
/answer 43d0-b allow
Control Request:
```yaml
...
arguments:
  path: /home/user/xyan/xy.ai.workbench/mcpc/src/xy/ai/mcpc/tools/ast/outline.py
  start: '"""``python_ast_outline``'
  end: functions.register(python_ast_outline)
  content: |-
  ...
  reason: "FileOutline und OutlineResult neu strukturieren: erfolgreiche Ergebnisse getrennt von Fehlern, und den Outline-Baum als typisierte, rekursive OutlineNode-Struktur mit Typ, Qualified Name, Zeilenbereich, Signatur und Docstring statt flacher Dict-Listen."
```
/answer 43d0-d allow
ReasoningToken: 213
Die Datei ist umgebaut. Kurzer Überblick der Änderungen:

- **`OutlineResult`**: hat jetzt `files: list[FileOutline]` (nur Erfolge) und `failed: list[OutlineFailure]` (Fehler als `path`/`error`-Objekte); `all_ok` entfällt, da durch `bool(failed)` überflüssig.
- **`FileOutline`**: enthält nur noch `path`, `stats` und `nodes` – kein `ok`/`error` mehr, da Fehlerfälle jetzt ausschließlich über `OutlineFailure` in `failed` abgebildet werden.
- **`OutlineNode`** (neu, typisiertes `dataclass`, kein `dict`): `type`, `qualified_name`, `lines` (Zeilennummer oder `"start-end"`), `signature` (inkl. Decorators, Basisklassen bzw. Argument-/Rückgabetyp), `docstring`, `children` – rekursiv für verschachtelte Klassen aufgeschlüsselt, alle direkten Modulkinder werden erfasst (nicht nur Imports/Klassen/Funktionen).
- Tool-`output_schema` nutzt `$defs`/`$ref` für die rekursive Node-Struktur; `handle()` serialisiert über `dataclasses.asdict`.
