Verbessere `/home/user/xyan/xy.ai.workbench/mcpc/src/xy/ai/mcpc/tools/colgrep/__init__.py`.

Die Option `full_content` soll entfernt werden. Es besteht nur die Wahl zwischen Files Only und der Code Anzeige der ersten 100 Zeichen.

Abhängig von der Länge der Query soll die Suche angepasst werden. Kleiner und gleich 4 Zeichen und es wird nur semantisch gesucht (semantic-only). Größer als 4 Zeichen bis 10 Zeichen und das Scoring wird von 0,9 Richtung 0,6 Schrittweise verschoben.
Argumente: `--semantic-only` und `--alpha 0.6` (Default) wo 0 ausschließlich Keywordsuche und 1 das Scoring zur semantischen Suche verschiebt.

Der Parameter "count" ist nicht relevant und kann entfernt werden.
SystemInit:  id=f366a73a-87f3-47a2-9f88-29c293b75c6f, cwd=/home/user/xyan/xy.ai.workbench, model=claude-sonnet-5
Control Request:
```yaml
id: ce5d-1
toolName: read_file
arguments:
  ...
  reason: "Aktuellen Code lesen, um full_content zu entfernen und Query-Länge-basierte Suche zu implementieren"
```
/answer ce5d-1 allow
ReasoningToken: 10
Control Request:
...
toolName: ask_user
arguments:
  question: "Soll der Parameter `semantic_only` (manuelle Steuerung durch den Aufrufer) komplett entfallen, weil ab jetzt automatisch anhand der Query-Länge entschieden wird (≤4 Zeichen → semantic-only, 5–10 Zeichen → alpha 0,9→0,6, >10 Zeichen → alpha 0,6)? Oder soll `semantic_only` als optionaler manueller Override erhalten bleiben und nur als Default automatisch gesetzt werden?"
  reason: "Klärung, ob der bisherige manuelle semantic_only-Parameter durch die automatische Query-Längen-Logik ersetzt oder als Override beibehalten werden soll, bevor der Code geändert wird."
```
/answer ce5d-2 deny Ja wir entfernen ihn aus der Steuerbarkeit des Agenten
ReasoningToken: 1373
Control Request:
```yaml
id: ce5d-3
...
arguments:
  path: /home/user/xyan/xy.ai.workbench/mcpc/src/xy/ai/mcpc/tools/colgrep/__init__.py
  start: '"""colgrep tool - semantic + keyword code search over a pre-built colgrep index.'
  end: functions.register(colgrep)
  content: |-
  ...
  reason: "full_content entfernen, count entfernen, automatische Query-Längen-basierte Steuerung von semantic-only/alpha implementieren"
```
/answer ce5d-3 allow
Cannot read STDOUT: the CLI process is not started (uuid=f366a73a-87f3-47a2-9f88-29c293b75c6f, processAlive=false). A prompt must be sent (writeLine) before reading.