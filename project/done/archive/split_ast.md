`/home/user/xyan/xy.ai.workbench/mcpc/src/xy/ai/mcpc/tools/ast/python/__init__.py` is zu groß. Splitte und extrahiere die semantischen Komponenten in separate Dateien im gleichen Package. Import müssen absolut bleiben.
SystemInit:  id=db3bfb04-204d-44ee-b30a-6527f44badd5, cwd=/home/user/xyan/xy.ai.workbench, model=claude-sonnet-5
Control Request:
```yaml
id: 0af9-1
toolName: read_file
arguments:
  ...
  reason: "Datei lesen, um sie zu analysieren und in semantische Komponenten aufzuteilen."
```
/answer 0af9-1 allow
Control Request:
```yaml
...
arguments:
  ...
  reason: "Basis-Modul verstehen (Engine, Located, Tree, id_segment, SEGMENT_MAX_CHARS), um Imports korrekt zu gestalten."
```
/answer 0af9-2 deny andere Dateien sind nicht relevant, exportierte Signatur und Exports müssen gleich bleiben
ReasoningToken: 246
Control Request:
```yaml
id: 0af9-3
    ...
    def comments_to_annotations(source: str) -> str:
        ...
        for i, line in enumerate(lines, start=1):
            ...
            if i in replaces:
                ...
                out.append(line)
        return ''.join(out)
  reason: Extraktion der Comment-zu-Annotation-Konvertierungslogik in eine eigene Datei desselben Packages.
```
/answer 0af9-3 allow
Control Request:
```yaml
...
arguments:
  ...
  reason: Extraktion der AST-Knoten-Klassifizierungs- und Gruppierungslogik in eine eigene Datei desselben Packages.
```
/answer 0af9-4 allow
Control Request:
```yaml
...
arguments:
  ...
  content: |
    ...
    ENGINE = PythonEngine()
  reason: "Extraktion der PythonEngine-Klasse und der Engine-Instanz in eine eigene Datei desselben Packages, mit absoluten Imports der zuvor ausgelagerten Komponenten."
```
/answer 0af9-5 allow
ReasoningToken: 8208
Control Request:
...
arguments:
  ...
  reason: "Reduktion der __init__.py auf ein Facade-Modul, das die extrahierten semantischen Komponenten mit unveränderter öffentlicher Signatur re-exportiert."
```
/answer 0af9-6 allow
Result Stats:  id=db3bfb04-204d-44ee-b30a-6527f44badd5, total: 0, in: 145, out: 15728, reason: 0, read: 79504, write: 23274
Die Datei wurde in vier Module aufgeteilt (alle Imports absolut):

- **`_comments.py`** – Comment-zu-Annotation-Konvertierung (`comments_to_annotations` + private Helfer/Regexe).
- **`_nodes.py`** – Knoten-Klassifizierung/-Formatierung und Statement-Gruppierung (`import_names`, `_is_expandable`, `_decorators`, `_StatementGroup`, `_DEF_TYPES`, `_IMPORT_TYPES`).
- **`_engine.py`** – `PythonEngine`-Klasse und `ENGINE`-Instanz, importiert aus `_comments.py`, `_nodes.py` und `base.py`.
- **`__init__.py`** – schlanke Facade, re-exportiert `ENGINE`, `PythonEngine`, `comments_to_annotations`, `import_names` (öffentliche Signatur unverändert).
