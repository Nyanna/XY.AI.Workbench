...
public class LanguageToolClient {
	...
	private static final String DISABLED_RULES = "WHITESPACE_RULE,DOPPELTES_LEERZEICHEN,LEERZEICHEN_VOR_SATZZEICHEN";
	 ...
	 * '@' line / comment line) are checked first and, if any applies, the whole
	 * line is blanked. Only lines that are not claimed by a block rule are scanned
	 * with {@link #MARKUP_RE} - and only within that single line's bounds. This
	 * ordering (blocks before inline regex, one line at a time) makes it
	 * structurally impossible for the inline regex to run across an already-masked
	 * block boundary, which was the root cause of previous mis-masking around
	 * fenced code.
	 */
	...
	private String callLanguageTool(String text) {
		try {
			String body = "language=" + URLEncoder.encode(LANGUAGE, StandardCharsets.UTF_8) + "&disabledRules="
					+ URLEncoder.encode(DISABLED_RULES, StandardCharsets.UTF_8) + "&text="
					+ URLEncoder.encode(text, StandardCharsets.UTF_8);

			HttpRequest request = HttpRequest.newBuilder().uri(URI.create(ENDPOINT)).timeout(TIMEOUT)
