 ...
 *
 * <p>Request body: {@code {"approvals":[...]}} (may be empty for a plain
 * poll). Response body: {@code {"pending":[...]}}.
 */
public class ClaudeCodeControlClient {
	 ...
	 * (originally unchanged) JSON structure of an open request/result whose
	 * "id" matches one of the currently pending items at the control endpoint.
	 * If so, the modified "arguments" (request phase) or "result" (result
	 * phase) are submitted to the control endpoint.
	 *
	 * @return {@code true} when {@code rawJson} was recognised as a pending
	 *         item and forwarded as a control decision
	 */
