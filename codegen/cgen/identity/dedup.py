...
from cgen.identity.fingerprint import fingerprint_of
...
class DedupContext:
    ...
    def __init__(self):
        self.fingerprints: dict[int, str] = {}  # id(canonical node) -> fingerprint
        self._canonical_by_fingerprint: dict[str, Node] = {}  # anonymous nodes only
...
def _rebuild_children(node: Node, ctx: DedupContext) -> Node:
    ...
    if isinstance(node, ObjectNode):
        return ObjectNode(
            properties=tuple(ctx.canonicalize_edge(edge) for edge in node.properties),
            required=node.required,
        )
    if isinstance(node, ListNode):
        return ListNode(
            elements=tuple(ctx.canonicalize_edge(edge) for edge in node.elements),
            mixed=node.mixed,
        )
    ...
    if isinstance(node, CompositionNode):
        return CompositionNode(
            ...
            branches=tuple(ctx.canonicalize_edge(edge) for edge in node.branches),
